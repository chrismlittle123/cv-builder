    ...
    <div className="bg-white min-h-screen p-8">
      <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
        <header className="text-center py-4">
          ...
          <p className="text-sm text-blue-600 mt-2">Contact: {cvData.profile.contact.email} | {cvData.profile.contact.phone}</p>
        </header>
            ...
            <section className="mb-12">
              ...
              {cvData.education.map((edu: any, index: number) => (
                <div key={index} className="mt-6">
                  ...
                  <p className="text-sm text-blue-800">Start: {edu.startDate} - End: {edu.endDate}</p>
                </div>
              ))}
            </section>
            <section className="mb-12">
                ...
                <div key={index} className="mt-6">
                  ...
                  <ul className="list-disc list-inside">
                    {skill.items.map((item: any, idx: number) => (
                      <li key={idx} className="text-sm text-blue-800">{item.name} (Proficiency: {item.proficiency})</li>
                    ))}
                  </ul>
                </div>
              ))}
            ...
            <section className="mb-12">
              ...
              {cvData.experience.map((exp: any, index: number) => (
                <div key={index} className="mt-6">
                  ...
                  <p className="text-sm text-blue-800">{exp.startDate} - {exp.endDate || 'Present'}</p>
